...
public class UploaderController : ControllerBase
{
    ...
    /// <summary>
    /// 服务器端上传：供内部应用服务器调用，直接传入文件字节流（最大60MB）
    /// </summary>
    [HttpPost]
    [RequestSizeLimit(60_000_000)]
    public async Task<ActionResult<Uri>> UploadByServer([FromForm] UploadRequest request, CancellationToken cancellationToken = default)
    {
        var file = request.File;
        string fileName = file.FileName;
        using Stream stream = file.OpenReadStream();
